class SprintsController < ApplicationController
  ...
  before_filter :find_model_object,
                :only => [:show, :edit, :update, :destroy, :edit_effort, :update_effort, :burndown, :sort]
  before_filter :find_project_from_association,
                :only => [:show, :edit, :update, :destroy, :edit_effort, :update_effort, :burndown]
  before_filter :find_project_by_project_id,
                :only => [:index, :new, :create, :change_task_status, :burndown_index, :sort]
  ...
  def sort
	  pbi_order = params[:pbi_order].split(',').map(&:to_i)
	  @sprint.pbis.each do |pbi|
		  pbi.init_journal(User.current)
		  pbi.position = pbi_order.index(pbi.id) + 1
		  pbi.save!
	  end
	  render :nothing => true
  end
