      ...
      base.class_eval do
        ...
        def tasks_by_status_id
          ...
          IssueStatus.task_statuses.each do |status|
            statuses[status.id] = descendants.select{|issue| (issue.status == status) and issue.visible?}
          end
          statuses
        end
        ...
        def post_it_css_class(options = {})
          classes = ["post-it", "big-post-it", tracker.post_it_css_class, "issue-status-#{status.id}"]
          if is_pbi?
            classes << "sprint-pbi"
