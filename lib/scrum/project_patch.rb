  ...
  module ProjectPatch
    def self.included(base)
      base.class_eval do
        ...
        def shared_sprints
	        if new_record?
		        Sprint.includes(:project).where("#{Project.table_name}.status <> ? ", STATUS_ARCHIVED)
	        else
            r = root? ? self : root
            Sprint.includes(:project).where(is_product_backlog: false).where("#{Project.table_name}.id = #{id}" +
              " OR (#{Project.table_name}.status <> #{Project::STATUS_ARCHIVED} AND (" +
              " (#{Project.table_name}.lft >= #{r.lft} AND #{Project.table_name}.rgt <= #{r.rgt} AND #{Sprint.table_name}.sharing = 'tree')" +
        ...
        def shared_sprints_and_product_backlog
	        ...
	        sprints << shared_product_backlog unless shared_product_backlog.nil?
	        sprints
        end
